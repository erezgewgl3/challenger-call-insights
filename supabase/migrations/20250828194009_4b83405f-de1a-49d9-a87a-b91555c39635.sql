-- Final Supabase Performance Cleanup - Fix Remaining Issues (Corrected Syntax)
-- Focus only on the specific multiple policy issues identified

-- 1. FIX INVITES TABLE - Combine multiple policies
...
-- 3. FIX SYSTEM_INTEGRATION_CONFIGS TABLE - Combine multiple policies  
...
-- Create single unified policy (public read + admin modify)
CREATE POLICY "system_integration_configs_unified_access" ON system_integration_configs
  FOR ALL USING (true)  -- Public can read
  WITH CHECK (
    EXISTS (SELECT 1 FROM users WHERE id = (select auth.uid()) AND role = 'admin')
...
-- Recreate with a better name (if it doesn't already exist)
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conname = 'integration_configs_unique_user_type_key'
    ) THEN
        ALTER TABLE integration_configs 
